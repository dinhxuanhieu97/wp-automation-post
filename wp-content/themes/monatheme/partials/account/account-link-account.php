...
$current_user_id = $args['current_userid'];
$acessTokenFB =  get_user_meta($current_user_id, 'fb_user_access_token', true);
?>
<div class="connect">
    <div class="connect_wrap">
        <p class="note-text"><?php echo __('Sau khi liên kết, bạn có thể đăng nhập bằng tài khoản mạng xã hội', 'monamedia'); ?></p>
        <ul class="connect_list">
            <?php
            if (class_exists('NextendSocialLogin', false)) {
                echo NextendSocialLogin::renderLinkAndUnlinkButtons('', true, true);
                $providerID                       = 'facebook';
                $authOptions['access_token_data'] = $acessTokenFB;
                $provider = NextendSocialLogin::getProviderByProviderID($providerID);
                $social_name = $provider->getAuthUserDataByAuthOptions('name', $authOptions);
                wp_localize_script(
                    'mona-frontend',
                    'label',
                    array(
                        'fb' => empty($social_name) ? 'Facebook' : $social_name,
                    )
                );
            }
            ...
            <div class="nsl-container nsl-container-block" data-align="left">
                <div class="nsl-container-buttons">
                    <a href="javascript:;" data-action="link">
                        <div class="nsl-button nsl-button-default nsl-button-facebook" data-skin="black" style="background-color:#000;">
                            ...
                            <div class="name-provider"><?php echo __('Google', 'monamedia'); ?></div>
                            <div class="nsl-button-label-container"><?php echo __('Liên kết ngay', 'monamedia'); ?></div>
                        </div>
                    </a>
                </div>
            </div>
